...
int pagerankLevelwiseOmpLoopU(vector<T>& a, vector<T>& r, vector<T>& c, const vector<T>& f, const vector<OK>& vfrom, const vector<K>& efrom, const vector<K>& vdata, K i, const J& ns, K N, T p, T E, int L, int EF) {
  float l = 0;
...
PagerankResult<T> pagerankLevelwiseOmp(const G& x, const H& xt, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}, const PagerankData<G> *C=nullptr) {
  ...
  return pagerankOmp(xt, ks, 0, ns, pagerankLevelwiseOmpLoopU<O, D, K, OK, T, decltype(ns)>, q, o);
}
...
PagerankResult<T> pagerankLevelwiseOmpDynamic(const G& x, const H& xt, const G& y, const H& yt, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}, const PagerankData<G> *C=nullptr) {
  ...
  return pagerankOmp(yt, ks, 0, ns, pagerankLevelwiseOmpLoop<O, D, T, decltype(ns)>, q, o);
}
