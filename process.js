...
const RRESLT = /^\[(.+?)\/(.+?) ms; (.+?) iters\.\] \[(.+?) err\.; (\d+) early\] (\w+)(?:\s+\{sleep_th: (\d+), sleep_dur: (\d+) ms\})?/m;
...
function readLogLine(ln, data, state) {
  if (RGRAPH.test(ln)) {
    ...
    var [, corrected_time, time, iterations, error, early_exit, technique, sleeping_threads, sleep_duration] = RRESLT.exec(ln);
    data.get(state.graph).push(Object.assign({}, state, {
      ...
      technique,
      sleeping_threads:  parseFloat(sleeping_threads || '0'),
