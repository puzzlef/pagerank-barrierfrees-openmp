...
void runPagerank(const G& x, const H& xt, int repeat) {
  ...
  for (float tolerance=1e-1; tolerance>=1e-15; tolerance/=10) {
    ...
    auto a1 = pagerankMonolithicSeq<false, false>(x, xt, init, {repeat, L1, damping, tolerance});
    ...
    auto a2 = pagerankMonolithicOmp<false, false>(x, xt, init, {repeat, L1, damping, tolerance});
    ...
    auto a3 = pagerankMonolithicOmp<true, false>(x, xt, init, {repeat, L1, damping, tolerance});
    ...
    auto a4 = pagerankBarrierfreeOmp<true, false>(x, xt, init, {repeat, L1, damping, tolerance});
    auto e4 = l1Norm(a4.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankBarrierfreeOmp {tol_norm: L1, tolerance: %.0e}\n", a4.time, a4.iterations, e4, tolerance);
  }

  // Use L2-norm for convergence check.
  for (float tolerance=1e-1; tolerance>=1e-15; tolerance/=10) {
    // Find pagerank using a single thread (unordered, no dead ends).
    auto a1 = pagerankMonolithicSeq<false, false>(x, xt, init, {repeat, L2, damping, tolerance});
    auto e1 = l1Norm(a1.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankSeqUnordered   {tol_norm: L2, tolerance: %.0e}\n", a1.time, a1.iterations, e1, tolerance);
    // Find pagerank accelerated with OpenMP (unordered, no dead ends).
    auto a2 = pagerankMonolithicOmp<false, false>(x, xt, init, {repeat, L2, damping, tolerance});
    auto e2 = l1Norm(a2.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankOmpUnordered   {tol_norm: L2, tolerance: %.0e}\n", a2.time, a2.iterations, e2, tolerance);
    // Find pagerank accelerated with OpenMP (ordered, no dead ends).
    auto a3 = pagerankMonolithicOmp<true, false>(x, xt, init, {repeat, L2, damping, tolerance});
    auto e3 = l1Norm(a3.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankOmpOrdered     {tol_norm: L2, tolerance: %.0e}\n", a3.time, a3.iterations, e3, tolerance);
    // Find pagerank with barrier-free iterations accelerated with OpenMP (ordered, no dead ends).
    auto a4 = pagerankBarrierfreeOmp<true, false>(x, xt, init, {repeat, L2, damping, tolerance});
    auto e4 = l1Norm(a4.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankBarrierfreeOmp {tol_norm: L2, tolerance: %.0e}\n", a4.time, a4.iterations, e4, tolerance);
  }
  ...
  for (float tolerance=1e-1; tolerance>=1e-15; tolerance/=10) {
    ...
    // Find pagerank accelerated with OpenMP (unordered, no dead ends).
    auto a2 = pagerankMonolithicOmp<false, false>(x, xt, init, {repeat, Li, damping, tolerance});
    auto e2 = l1Norm(a2.ranks, a1.ranks);
    ...
    auto e4 = l1Norm(a4.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankBarrierfreeOmp {tol_norm: Li, tolerance: %.0e}\n", a4.time, a4.iterations, e4, tolerance);
  }
}
